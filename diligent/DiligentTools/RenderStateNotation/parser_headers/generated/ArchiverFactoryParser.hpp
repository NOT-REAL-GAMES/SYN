...
inline void WriteRSN(nlohmann::json &Json,
                     const SerializationDeviceD3D11Info &Type,
...
inline void ParseRSN(const nlohmann::json &Json,
                     SerializationDeviceD3D11Info &Type,
                     DynamicLinearAllocator &Allocator) {
...
inline void WriteRSN(nlohmann::json &Json,
                     const SerializationDeviceVkInfo &Type,
                     DynamicLinearAllocator &Allocator) {
  ...
  if (!SafeStrEqual(Type.DxCompilerPath,
                    SerializationDeviceVkInfo{}.DxCompilerPath))
    WriteRSN(Json["DxCompilerPath"], Type.DxCompilerPath, Allocator);
}

inline void ParseRSN(const nlohmann::json &Json,
                     SerializationDeviceVkInfo &Type,
                     DynamicLinearAllocator &Allocator) {
  NLOHMANN_JSON_VALIDATE_KEYS(Json, {
                                        "ApiVersion",
                                        "SupportsSpirv14",
...
inline void WriteRSN(nlohmann::json &Json,
                     const SerializationDeviceMtlInfo &Type,
                     DynamicLinearAllocator &Allocator) {
  if (!SafeStrEqual(Type.CompileOptionsMacOS,
                    SerializationDeviceMtlInfo{}.CompileOptionsMacOS))
    WriteRSN(Json["CompileOptionsMacOS"], Type.CompileOptionsMacOS, Allocator);

  if (!SafeStrEqual(Type.CompileOptionsiOS,
                    SerializationDeviceMtlInfo{}.CompileOptionsiOS))
    WriteRSN(Json["CompileOptionsiOS"], Type.CompileOptionsiOS, Allocator);

  if (!SafeStrEqual(Type.MslPreprocessorCmd,
                    SerializationDeviceMtlInfo{}.MslPreprocessorCmd))
    WriteRSN(Json["MslPreprocessorCmd"], Type.MslPreprocessorCmd, Allocator);

  if (!SafeStrEqual(Type.DumpDirectory,
                    SerializationDeviceMtlInfo{}.DumpDirectory))
    WriteRSN(Json["DumpDirectory"], Type.DumpDirectory, Allocator);
}

inline void ParseRSN(const nlohmann::json &Json,
                     SerializationDeviceMtlInfo &Type,
                     DynamicLinearAllocator &Allocator) {
  NLOHMANN_JSON_VALIDATE_KEYS(Json, {
                                        "CompileOptionsMacOS",
                                        "CompileOptionsiOS",
...
inline void WriteRSN(nlohmann::json &Json,
                     const SerializationDeviceCreateInfo &Type,
                     DynamicLinearAllocator &Allocator) {
...
inline void ParseRSN(const nlohmann::json &Json,
                     SerializationDeviceCreateInfo &Type,
                     DynamicLinearAllocator &Allocator) {
  NLOHMANN_JSON_VALIDATE_KEYS(Json, {
                                        "DeviceInfo",
                                        "AdapterInfo",
