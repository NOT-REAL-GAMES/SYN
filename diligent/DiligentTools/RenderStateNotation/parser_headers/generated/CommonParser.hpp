...
#define NLOHMANN_JSON_SERIALIZE_ENUM_EX(ENUM_TYPE, ...)                        \
  inline void to_json(nlohmann::json &j, const ENUM_TYPE &e) {                 \
    ...
    if (it == std::end(m))                                                     \
      throw nlohmann::json::other_error::create(                               \
          JsonInvalidEnum,                                                     \
          std::string("invalid enum value for " #ENUM_TYPE ""), j);            \
    j = it->second;                                                            \
  }                                                                            \
  inline void from_json(const nlohmann::json &j, ENUM_TYPE &e) {               \
    ...
    if (it == std::end(m))                                                     \
      throw nlohmann::json::other_error::create(                               \
          JsonInvalidEnum,                                                     \
          std::string("invalid enum value for " #ENUM_TYPE ": ") +             \
              j.get<std::string>(),                                            \
          j);                                                                  \
    e = it->first;                                                             \
  }
  ...
  do {                                                                         \
    ...
    for (auto it = JSON.begin(); it != JSON.end(); ++it) {                     \
      auto result = std::find_if(                                              \
          std::begin(m), std::end(m), [&it](const char *key) -> bool {         \
            return std::strcmp(it.key().c_str(), key) == 0;                    \
          });                                                                  \
      if (result == std::end(m))                                               \
        throw nlohmann::json::other_error::create(                             \
            JsonUnexpectedKey, std::string("unexpected key: ") + it.key(),     \
            JSON);                                                             \
    }                                                                          \
  } while (false)

void WriteRSN(nlohmann::json &Json, const ShaderMacro &Type,
              DynamicLinearAllocator &Allocator);

void ParseRSN(const nlohmann::json &Json, ShaderMacro &Type,
              DynamicLinearAllocator &Allocator);

template <typename Type,
          std::enable_if_t<!std::is_pointer<Type>::value, bool> = true>
inline void WriteRSN(nlohmann::json &Json, const Type &Object,
                     DynamicLinearAllocator &Allocator) {
  Json = Object;
}
...
template <typename Type,
          std::enable_if_t<!std::is_pointer<Type>::value, bool> = true>
inline void ParseRSN(const nlohmann::json &Json, Type &pObject,
                     DynamicLinearAllocator &Allocator) {
  Json.get_to(pObject);
}

template <typename Type>
inline void WriteRSN(nlohmann::json &Json, const Type *const pObject,
                     DynamicLinearAllocator &Allocator) {
  WriteRSN(Json, *pObject, Allocator);
}

template <typename Type>
inline void ParseRSN(const nlohmann::json &Json, const Type *&pObject,
                     DynamicLinearAllocator &Allocator) {
...
inline void WriteRSN(nlohmann::json &Json, const Type *const pData,
                     TypeSize NumElements, DynamicLinearAllocator &Allocator) {
...
template <typename Type, typename TypeSize>
inline void ParseRSN(const nlohmann::json &Json, const Type *&pObjects,
                     TypeSize &NumElements, DynamicLinearAllocator &Allocator) {
  if (!Json.is_array())
    throw nlohmann::json::type_error::create(
        JsonTypeError,
        std::string("type must be array, but is ") + Json.type_name(), Json);
...
template <>
inline void WriteRSN(nlohmann::json &Json, const void *pData, size_t Size,
                     DynamicLinearAllocator &Allocator) {
  std::vector<uint8_t> PackedData(static_cast<const uint8_t *>(pData),
                                  static_cast<const uint8_t *>(pData) + Size);
  Json = nlohmann::json::binary(PackedData);
}

template <>
inline void ParseRSN(const nlohmann::json &Json, const void *&pObject,
                     size_t &Size, DynamicLinearAllocator &Allocator) {
...
template <>
inline void WriteRSN(nlohmann::json &Json, const char *const Str,
                     DynamicLinearAllocator &Allocator) {
  if (Str != nullptr)
    Json = Str;
}
...
template <>
inline void ParseRSN(const nlohmann::json &Json, const char *&Str,
                     DynamicLinearAllocator &Allocator) {
  Str = Allocator.CopyString(Json.get<std::string>());
}

inline void WriteRSN(nlohmann::json &Json, const ShaderMacroArray &Macros,
                     DynamicLinearAllocator &Allocator) {
...
inline void ParseRSN(const nlohmann::json &Json, ShaderMacroArray &Macros,
                     DynamicLinearAllocator &Allocator) {
  if (!Json.is_array())
    throw nlohmann::json::type_error::create(
        JsonTypeError,
...
inline void ParseRSN(const nlohmann::json &Json, const Char **&pObjects,
                     Uint32 &NumElements, DynamicLinearAllocator &Allocator) {
...
template <typename Type>
inline void SerializeBitwiseEnum(nlohmann::json &Json, Type EnumBits,
                                 DynamicLinearAllocator &Allocator) {
...
template <typename Type>
inline void DeserializeBitwiseEnum(const nlohmann::json &Json, Type &EnumBits,
                                   DynamicLinearAllocator &Allocator) {
  ...
  if (Json.is_array()) {
    ...
    throw nlohmann::json::type_error::create(
        JsonTypeError,
        std::string("type must be array or string, but is ") + Json.type_name(),
        Json);
  }
}
...
inline void SerializeConstArray(nlohmann::json &Json,
                                const Type (&pObjects)[NumElements],
                                DynamicLinearAllocator &Allocator) {
  for (size_t i = 0; i < NumElements; i++)
...
template <typename Type, size_t NumElements,
          std::enable_if_t<std::is_arithmetic<Type>::value, bool> = true>
inline void DeserializeConstArray(const nlohmann::json &Json,
                                  Type (&pObjects)[NumElements],
                                  DynamicLinearAllocator &Allocator) {
  if (!Json.is_array())
    throw nlohmann::json::type_error::create(
        JsonTypeError,
...
inline void DeserializeConstArray(const nlohmann::json &Json,
  ...
  if (!Json.is_object())
    throw nlohmann::json::type_error::create(
        JsonTypeError,
        std::string("type must be object, but is ") + Json.type_name(), Json);
...
template <typename Type, size_t NumElements>
inline bool CompareConstArray(const Type (&Lhs)[NumElements],
                              const Type (&Rhs)[NumElements]) {
  for (size_t i = 0; i < NumElements; i++)
    if (!(Lhs[i] == Rhs[i]))
      return false;
  return true;
