...
inline void WriteRSN(nlohmann::json& Json, const RenderPassAttachmentDesc& Type, DynamicLinearAllocator& Allocator)
{
...
inline void ParseRSN(const nlohmann::json& Json, RenderPassAttachmentDesc& Type, DynamicLinearAllocator& Allocator)
{
...
inline void WriteRSN(nlohmann::json& Json, const AttachmentReference& Type, DynamicLinearAllocator& Allocator)
{
...
inline void ParseRSN(const nlohmann::json& Json, AttachmentReference& Type, DynamicLinearAllocator& Allocator)
{
    NLOHMANN_JSON_VALIDATE_KEYS(Json, {
                                          "AttachmentIndex",
                                          "State",
...
inline void WriteRSN(nlohmann::json& Json, const ShadingRateAttachment& Type, DynamicLinearAllocator& Allocator)
{
...
inline void ParseRSN(const nlohmann::json& Json, ShadingRateAttachment& Type, DynamicLinearAllocator& Allocator)
{
    NLOHMANN_JSON_VALIDATE_KEYS(Json, {
                                          "Attachment",
                                          "TileSize",
...
inline void WriteRSN(nlohmann::json& Json, const SubpassDesc& Type, DynamicLinearAllocator& Allocator)
{
    if (!(Type.pInputAttachments == SubpassDesc{}.pInputAttachments))
        WriteRSN(Json["pInputAttachments"], Type.pInputAttachments, Type.InputAttachmentCount, Allocator);

    if (!(Type.pRenderTargetAttachments == SubpassDesc{}.pRenderTargetAttachments))
        WriteRSN(Json["pRenderTargetAttachments"], Type.pRenderTargetAttachments, Type.RenderTargetAttachmentCount, Allocator);

    if (!(Type.pResolveAttachments == SubpassDesc{}.pResolveAttachments))
        WriteRSN(Json["pResolveAttachments"], Type.pResolveAttachments, Type.PreserveAttachmentCount, Allocator);

    if (!(Type.pDepthStencilAttachment == SubpassDesc{}.pDepthStencilAttachment))
        WriteRSN(Json["pDepthStencilAttachment"], Type.pDepthStencilAttachment, Allocator);

    if (!(Type.pPreserveAttachments == SubpassDesc{}.pPreserveAttachments))
        WriteRSN(Json["pPreserveAttachments"], Type.pPreserveAttachments, Type.PreserveAttachmentCount, Allocator);

    if (!(Type.pShadingRateAttachment == SubpassDesc{}.pShadingRateAttachment))
        WriteRSN(Json["pShadingRateAttachment"], Type.pShadingRateAttachment, Allocator);
}

inline void ParseRSN(const nlohmann::json& Json, SubpassDesc& Type, DynamicLinearAllocator& Allocator)
{
    NLOHMANN_JSON_VALIDATE_KEYS(Json, {
    ...
    if (Json.contains("pInputAttachments"))
        ParseRSN(Json["pInputAttachments"], Type.pInputAttachments, Type.InputAttachmentCount, Allocator);

    if (Json.contains("pRenderTargetAttachments"))
        ParseRSN(Json["pRenderTargetAttachments"], Type.pRenderTargetAttachments, Type.RenderTargetAttachmentCount, Allocator);

    if (Json.contains("pResolveAttachments"))
        ParseRSN(Json["pResolveAttachments"], Type.pResolveAttachments, Type.PreserveAttachmentCount, Allocator);

    if (Json.contains("pDepthStencilAttachment"))
        ParseRSN(Json["pDepthStencilAttachment"], Type.pDepthStencilAttachment, Allocator);

    if (Json.contains("pPreserveAttachments"))
        ParseRSN(Json["pPreserveAttachments"], Type.pPreserveAttachments, Type.PreserveAttachmentCount, Allocator);

    if (Json.contains("pShadingRateAttachment"))
        ParseRSN(Json["pShadingRateAttachment"], Type.pShadingRateAttachment, Allocator);
}

inline void WriteRSN(nlohmann::json& Json, const SubpassDependencyDesc& Type, DynamicLinearAllocator& Allocator)
{
...
inline void ParseRSN(const nlohmann::json& Json, SubpassDependencyDesc& Type, DynamicLinearAllocator& Allocator)
{
    NLOHMANN_JSON_VALIDATE_KEYS(Json, {
                                          "SrcSubpass",
                                          "DstSubpass",
...
inline void WriteRSN(nlohmann::json& Json, const RenderPassDesc& Type, DynamicLinearAllocator& Allocator)
{
    ...
    if (!(Type.pAttachments == RenderPassDesc{}.pAttachments))
        WriteRSN(Json["pAttachments"], Type.pAttachments, Type.AttachmentCount, Allocator);
    ...
    if (!(Type.pDependencies == RenderPassDesc{}.pDependencies))
        WriteRSN(Json["pDependencies"], Type.pDependencies, Type.DependencyCount, Allocator);
}

inline void ParseRSN(const nlohmann::json& Json, RenderPassDesc& Type, DynamicLinearAllocator& Allocator)
{
    NLOHMANN_JSON_VALIDATE_KEYS(Json, {
                                          "Name",
                                          "AttachmentCount",
    ...
    if (Json.contains("pAttachments"))
        ParseRSN(Json["pAttachments"], Type.pAttachments, Type.AttachmentCount, Allocator);
    ...
    if (Json.contains("pDependencies"))
        ParseRSN(Json["pDependencies"], Type.pDependencies, Type.DependencyCount, Allocator);
}
